...
/* nudged up a touch: the lungs carry more weight below their centre than above,
   so a box-centred mark reads as sitting low next to the wordmark */
.sky-title h1 .mark{width:31px;height:31px;flex:0 0 auto;margin:-4px -2px 0 -4px;
  filter:drop-shadow(0 1px 2px rgba(8,54,69,.28));}
...
/* a pill that is actually a button, so it reads as the thing to tap */
.pill.act{border:none;background:var(--accent-soft);color:var(--accent);cursor:pointer;}
...
/* the enlarge control, and the sheet it opens */
.ttlacts{display:flex;align-items:center;gap:8px;flex:0 0 auto;}
.expandbtn{width:30px;height:30px;flex:0 0 auto;border:none;border-radius:50%;background:var(--paper-2);
  color:var(--ink-2);font-size:14px;line-height:1;display:flex;align-items:center;justify-content:center;padding:0;}
.zoom{position:fixed;inset:auto 0 0 0;z-index:82;max-width:720px;margin:0 auto;background:#fff;
  border-radius:26px 26px 0 0;padding:20px 16px calc(24px + env(safe-area-inset-bottom));
  box-shadow:0 -20px 50px -20px rgba(8,54,69,.5);animation:slideup .34s var(--ease) both;}
.zoomhead{display:flex;align-items:center;gap:10px;margin-bottom:14px;}
.zoomhead b{flex:1;font-family:var(--display);font-size:17px;font-weight:600;letter-spacing:-0.02em;}
/* pan-x here and nowhere above it: this is a scroller, not a day-swipe surface */
.zoombody{overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;touch-action:pan-x;}
.zoombody .scrollx{overflow:visible;}
.zoomnote{margin-top:12px;}
...
const SEED_TAGS = ["Household illness", "Travel or flight", "Poor sleep", "Air quality", "Saw the doctor", "Missed airway care", "Plug or cast"];
const SEED_PRN = [];
...
// The lungs from the home screen icon, at 96px with the background taken out, so
...
const emptyState = () => ({ v: 8, days: {}, courses: [], tags: [...SEED_TAGS], prnMeds: [...SEED_PRN], customDrugs: [], customSymptoms: [], locations: [...SEED_LOCATIONS], regimen: JSON.parse(JSON.stringify(SEED_REGIMEN)), rescue: [], appt: { date: "", who: "" }, questions: [...SEED_QUESTIONS], track: defaultTracking(), remindTimes: [], conditions: [], weightUnit: "kg", meta: {} });
...
function migrate(raw) {
  ...
  // is turned into the tag so it is not lost with the field.
  if (fromVersion < 8) {
    Object.keys(s.days).forEach((k) => {
      if (s.days[k].cast) {
        s.days[k].tags = s.days[k].tags || [];
        if (!s.days[k].tags.includes("Plug or cast")) s.days[k].tags.push("Plug or cast");
      }
      delete s.days[k].cast;
    });
    if (!s.tags.includes("Plug or cast")) s.tags.push("Plug or cast");
  }
  s.v = 8;
  return s;
}
      ...
      .map((g) => ({
        ...
        taken: g.courses
          .map((c) => {
            ...
            return a ? { drug: c.drug, taken: a.taken, expected: a.expected } : null;
          })
          .filter(Boolean),
      }))
        ...
        {finishedCourses.map((c) => (
          <div key={c.id} style={{ paddingTop: 14 }}>
            ...
            <div className="course" style={{ paddingBottom: 4 }}>
              ...
              <button className="pill act" onClick={() => setReviewing(addDays(c.startDate, c.days - 1))}>
                review
              </button>
              <div className="rowacts">
                <button
            ...
            {/* the outcome and the note used to be asked again here, which meant
                the prompt at the top opened a sheet and this asked the same
                questions a long scroll further down. The sheet is the only place
                that asks now; this row just opens it. */}
          </div>
        ))}
      </div>
    </>
  ...
  return (
    <div className="card">
      <div className="card-t">
        ...
        <span className="ttlacts">
          <Seg options={SPANS} value={span} onChange={onSpan} small />
          <Expand title="Side by side">{chart}</Expand>
        </span>
      </div>
      {chart}
    </div>
...
// A chart on a phone is a few hundred pixels wide holding a year of days. This
// opens the same chart full screen at whatever width you pick and lets you drag
// along it — the SVG scales its text and strokes with the viewBox, so it is a
// real enlargement rather than a blurry one.
function Expand({ title, children }) {
  const [open, setOpen] = useState(false);
  const [zoom, setZoom] = useState(200);
  const ZOOMS = [{ v: 100, label: "Fit" }, { v: 200, label: "2×" }, { v: 350, label: "3×" }];
  return (
    <>
      <button className="expandbtn" onClick={() => setOpen(true)} aria-label={"Enlarge " + title}>
        {"\u2921"}
      </button>
      {open && (
        <>
          <div className="scrim" onClick={() => setOpen(false)} />
          <div className="zoom" role="dialog" aria-modal="true">
            <div className="zoomhead">
              <b>{title}</b>
              <Seg options={ZOOMS} value={zoom} onChange={setZoom} small />
              <button className="btn sm" onClick={() => setOpen(false)}>Close</button>
            </div>
            <div className="zoombody">
              <div style={{ width: zoom + "%" }}>{children}</div>
            </div>
            <div className="note zoomnote">Drag sideways to move along it.</div>
          </div>
        </>
      )}
    </>
  ...
  return (
    <>
      ...
      <div className="modal" role="dialog" aria-modal="true">
        ...
        {/* what the log already knows, so the question below can be answered
            against something rather than from memory alone */}
        {group.taken.length > 0 && (
          <div className="insight" style={{ marginTop: 12 }}>
            {group.taken.map((t) => (
              <div key={t.drug}>
                {t.drug}: {t.taken} of {t.expected} doses ticked off
              </div>
            ))}
          </div>
        )}
    ...
    <div className="card">
      <div className="card-t">
        ...
        <span className="ttlacts">
          ...
          <Expand title="Your run-up">{runupChart}</Expand>
        </span>
      </div>
      {runupChart}
      {rows.map((lane) => {
        const at = (off) => {
          const c = lane.data.find((q) => q.off === off);
          return c && c.mean != null ? c.mean : null;
        };
        // Compared against the day BEFORE it opened, not the day itself. What
        // happens on day zero is the episode; what matters here is whether the
        // measure had already moved while you still thought you were well.
        const wk = at(-7);
        const eve = at(-1);
        if (wk == null || eve == null) return null;
        const d = eve - wk;
        const fmt = (v) => (Math.abs(v) >= 10 ? Math.round(v) : Math.round(v * 10) / 10);
        const moved = Math.abs(d) >= 0.4;
        return (
          <div className={"insight" + (moved ? " warn" : "")} key={lane.key} style={{ marginTop: 8 }}>
            <b>{lane.label}</b>
            <br />
            {moved
              ? `${fmt(wk)} a week out, ${fmt(eve)} the day before it opened — already ${
                  d > 0 ? "climbing" : "dropping"
                } while you still felt well.`
              : `Steady until the day itself. This one gives you no warning.`}
          </div>
        );
      })}
      <div className="note" style={{ marginTop: 11 }}>
        Averaged across your episodes. The shaded column is the day you marked yourself unwell — anything already
        moving to the left of it is warning you had at the time.
      </div>
    </div>
  );
}
    ...
    <div className="bx">
      ...
      >
        ...
        <div className="sky-in">
          <div className="sky-title">
            <h1>
              {tab === "today" && <img className="mark" src={MARK} alt="" />}
              {TITLES[tab][0]}
            </h1>
